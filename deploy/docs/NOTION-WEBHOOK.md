...
systemd timer 调用内容容器 `127.0.0.1:3031/api/revalidate`。定时任务不可用时，原有五分钟
ISR、Notion Worker/直连兜底、Redis 内容缓存和评论功能仍然工作。

## 前置条件

- starter 主站监听 `127.0.0.1:3030`，heo 内容站监听 `127.0.0.1:3031`。
- Webhook 入口仍由 `www` 的 `notionnext-app` 接收；共享队列只由
  `notionnext-way` 消费。
...
`.env.production`，删除 setup mode 和容器临时文件，只重建 `app`，再由
`way` 执行一次共享路由状态 bootstrap；仅当响应包含 `ok:true` 时才启用 timer。

正文修改只刷新 `way` 的文章和知识图谱。标题、摘要、Slug、分类、标签、
发布或删除等影响列表的修改会使现有路由计划包含 `/`；此时 `way` 通过
Docker 内网和相同 Bearer Token 刷新 `www` 首页。远程刷新失败会保留队列项，
下一分钟重试。
...
2. 如需回滚应用，用双站部署流程恢复两个容器的同一镜像 tag。
3. 不执行 `docker compose down -v`，不删除 `redis-data` 或
   `notion-cache`、`notion-cache-way` volume。
