...
评论身份由所选评论插件自己负责，不依赖 NotionNext 的站点登录：

- Giscus、Utterances 使用 GitHub 身份。
- Twikoo、Waline、Artalk 等使用各自服务提供的昵称、邮箱或账号机制。
- Notion Comments 使用评论表单中的昵称和邮箱，并按现有规则完成校验、限流与审核。

如果未来确实需要会员权限或“只有登录用户才能评论”，应重新设计独立的身份与权限层，再与评论插件做最小集成，不应把账户系统默认加入普通博客路径。
...
历史方案讨论见 [会员、权限与评论可选集成路线图](../../developer/MEMBERSHIP_COMMENTS_ROADMAP.md)，该文档不代表当前已实现功能。
