...
::: warning 注意
EdgeOne Pages 支持动态 Node.js 部署（`pnpm install` + `pnpm build`），而非静态导出。注意区分「云函数」与「静态站点」两种构建模式。
:::
...
1. **构建命令**：推荐 `pnpm install` + `pnpm build`，可启用云函数动态加载，同时避免 EdgeOne Pages 内存不足导致的构建失败问题。
...
EdgeOne Pages 上的 NotionNext 使用统一的 ISR（Incremental Static Regeneration）策略：

- **公开页面**（首页、归档、分类、标签、文章等）在构建时预先生成，默认在 **300 秒**后因访问而触发 ISR 刷新。
- **300 秒不是倒计时**：只有当缓存过期后，第一次访问才会触发页面重新生成。之后的访问继续使用缓存，直到下一个 300 秒周期结束。
- **Notion 编辑无需重新部署**：在 Notion 中发布、编辑或取消发布内容后，缓存会在 300 秒后自动反映变更；首次访问新发布的路由时可能会稍慢（触发 ISR 生成）。
- **以下内容保持动态**：评论、支付、订阅、文章密码验证、API 路由（`/api/*`）以及所有 mutations 均不受 ISR 缓存影响。
- **知识图谱独立刷新**：知识图谱保持 10 分钟的独立刷新周期，与公开页面 ISR 解耦。
- **EdgeOne 路由限制**：区域化页面数据重写（`/_next/data/:buildId/zh-CN.json` 等）仅限于 `/_next/data/*` 路径，**请勿扩展到 `/api/*`**。

::: warning 注意
EdgeOne 会自动为 ISR 响应添加 `s-maxage=300, stale-while-revalidate=86400` 缓存头，CDN 会在 300 秒后进行重新验证，同时允许最多一天的 stale 响应。无需手动配置缓存规则。
:::
