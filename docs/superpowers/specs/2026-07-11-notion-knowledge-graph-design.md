...
7. The function rebuilds the deduplicated graph, writes a unique immutable graph version, then creates an immutable publication marker only after that version write succeeds.
...
state/graph-publications/<window>-<id>.json
                                   Immutable marker for one completed graph version
...
The API strongly lists publication markers and selects the lexicographically greatest key. Each marker key embeds a fixed-width, zero-padded refresh-window number, with the generation ID as a deterministic tie-break. It strongly reads that marker, then strongly reads the immutable graph key stored in its value. A refresh first writes `graph/versions/<id>.json` with `onlyIfNew: true`, then creates `state/graph-publications/<padded-window>-<id>.json` with `onlyIfNew: true`. If marker acknowledgement is ambiguous, it can refer only to the graph version written first. A late completion from an older window cannot supersede a newer publication because the reader selects by window key, not write time.
...
The store retains the two newest publication markers and their versions. Cleanup removes an older marker before its version, never deletes the selected newest marker, and readers retry marker discovery once when cleanup races a marker read.
...
- Keep the latest completed publication when a new version write fails; a later refresh window may retry.
...
4. Verify Blob strong publication-marker listing, conditional refresh-window claims, immutable version/marker publication order, bounded cleanup, and marker resolution against the real EdgeOne Blob service, not only mocks.
