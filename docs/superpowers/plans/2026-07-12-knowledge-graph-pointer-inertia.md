...
- Stationary time before release is fixed at 80 ms.
- Inertia duration is capped at 240 ms and displacement at 80 screen pixels.
...
Use mocked pointer events, animation frames, `centerAt()` and `zoom()` to prove: quick background release moves the viewport in the release direction; stopping for 80 ms produces no motion; node dragging produces no inertia; movement stops by 240 ms and 80 px; reduced motion and cancellation inputs stop it.
...
Add named internal constants for 4 px tolerance, 80 ms idle cutoff, 240 ms duration, 80 px cap, a small release-speed threshold, and frame decay. Reuse the rendered radius calculation in `nodeCanvasObject` and `nodePointerAreaPaint` so visual and hit geometry cannot drift.
