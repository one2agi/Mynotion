...
const KnowledgeGraphDrawer = ({ isDarkMode, isOpen, onClose, post }) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/knowledge-graph')
      .then(async response => {
        ...
        return isPublicGraph(responseGraph)
          ? { graph: responseGraph, status: 'ready' }
          : { status: 'unavailable' }
      })
      .then(result => {
        if (cancelled) return
  ...
  const showEmpty = status === 'ready' && graph?.edges.length === 0
  ...
  return (
    <div className='fixed inset-0 z-50 flex justify-end'>
      ...
      <section
        aria-label='知识图谱'
        aria-modal='true'
        className={`relative flex h-full w-full max-w-[420px] flex-col border-l shadow-2xl ${panelColor}`}
        ref={dialogRef}
        role='dialog'
        tabIndex='-1'
      >
