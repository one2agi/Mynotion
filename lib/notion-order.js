...
// 注意：serverless 环境（Vercel/EdgeOne）项目目录是只读，必须用 /tmp
// 见 getDeadLetterPath() 的 JSDoc

/**
 * 获取死信文件路径：
 * - serverless 环境（Vercel/EdgeOne）：/tmp/orders-failed.json（项目目录只读，/tmp 可写）
 * - 本地 dev / 自建服务器：lib/orders-failed.json（项目目录可写）
 *
 * /tmp 在 serverless 上是实例级 ephemeral，容器重启会丢几分钟数据。
 * 这是可接受的（每月偶发 1-2 单需人工查日志补单，比 100% 丢单好）。
 *
 * @returns {string} 死信文件绝对路径
 */
function getDeadLetterPath() {
  if (process.env.VERCEL || process.env.EDGEONE) {
    return '/tmp/orders-failed.json'
  }
  return path.join(process.cwd(), 'lib', 'orders-failed.json')
}

/**
 * 写入死信文件：失败订单兜底
 * - serverless：写到 /tmp（容器重启会丢）
 * - 本地：写到 lib/orders-failed.json（持久）
 * - 写失败：console.warn（不抛错，保持 createOrderPage 永不抛错的契约）
 * @param {Object} entry
 */
function writeDeadLetter(entry) {
  ...
  try {
    ...
    // 死信文件本身写失败（边缘 case：/tmp 也满等），仅警告不抛出
 ...
 *   - 成功：pageId 字符串
 *   - 失败：null（已写入死信，运营可人工补单）
 *   - 抛错：未预期的 code bug（notify catch 会返回 'error' 让 Z-Pay 重试，
 *           重试耗尽 → 订单永久丢失）
 *
 * 任何错误路径都必须被本函数内部 catch：
 *   - Notion API 5xx/429/timeout → withRetry 重试 3 次后写入死信
 *   - 死信文件 fs.writeFileSync 失败 → console.warn + 返回 null
 *   - 死信文件 JSON.parse 失败 → catch 内吞掉 + 重新初始化数组
 *
...
module.exports = { createOrderPage, getNotionClient, writeDeadLetter, getDeadLetterPath }