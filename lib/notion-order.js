 ...
 *   发送产品链接 (url)             | Token (rich_text, 不写)
 *
 * 注：Token、源链接、备注、交付时间 由用户/运营手动填，本函数不写。
 *     发送产品链接由系统自动写入（来自 Token 数据库）。
 */
 ...
 * @param {string} [orderData.productLink] - 发送产品链接（来自 Token 数据库）
 * @param {Object} [client] - 仅用于测试，注入 mock client
...
async function createOrderPage(orderData, client) {
  const { productName, outTradeNo, email, name, amount, discountCode, paidAt, productLink } = orderData
  ...
  try {
    ...
    if (existing.results.length > 0) {
      // 幂等命中：订单已存在
      // 如果这次带了 productLink 且原订单没有，补写
      // 处理"上次 lookupUnusedToken 失败、Z-Pay 重试时拿到 token"的场景
    ...
    // 重试耗尽 → 推送死信 webhook（**必须 await**，否则 fire-and-forget 会在 notify.js
    // 返回 success 后还在跑，EdgeOne serverless 冻结实例时 webhook 可能根本没发完）
    const entry = {
...
/**
 * 原子地"读 + 增"订单的"重试次数"属性，返回新次数和 webhook 已推送标记
 * 用法：const { newCount, webhookPushed } = await incrementRetryCount(pageId, outTradeNo, client?)
 *
 * 设计说明：
 * - 返回 webhookPushed 让调用方决定是否推 webhook（避免重复推送）
 * - pageId 为空时返回 { newCount: 0, webhookPushed: false }，不阻塞主流程
 * - 失败时静默 + 日志（同 createOrderPage 的 catch 风格）
 *
 * 来源：2026-06-24 production log 显示 Z-Pay 重试 6 次 → count=5 和 count=6 都推了 webhook
 * 加 webhook_pushed 字段后，只在 5 次推一次，6+ 次不重复推
 *
 * @param {string|null} pageId - 订单的 Notion page ID
 * @param {string} outTradeNo - 商户订单号（用于日志）
 * @param {Object} [client] - 可选的 Notion client 注入（测试用）
 * @returns {Promise<{newCount: number, webhookPushed: boolean}>}
 */
async function incrementRetryCount(pageId, outTradeNo, client) {
  if (!pageId) return { newCount: 0, webhookPushed: false }
  const notion = getNotionClient(client)
  try {
    const order = await notion.pages.retrieve({ page_id: pageId })
    const currentCount = order.properties?.['重试次数']?.number || 0
    const newCount = currentCount + 1
    const webhookPushed = order.properties?.['webhook_pushed']?.checkbox === true

    await notion.pages.update({
      page_id: pageId,
      properties: { '重试次数': { number: newCount } }
    })
    return { newCount, webhookPushed }
  } catch (e) {
    console.error('[notion-order] incrementRetryCount 失败', { outTradeNo, error: e.message })
    return { newCount: 0, webhookPushed: false }
  }
}

module.exports = { createOrderPage, getNotionClient, incrementRetryCount }