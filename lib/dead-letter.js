...
const { timeoutSignal } = require('@/lib/abort')
...
/**
 * 把时间字符串格式化为北京时间显示
 * 无效输入返回 'N/A'，不抛错（保持 MUST NEVER reject 契约）
 *
 * 支持两种输入：
 * 1. ISO 8601（带时区）— 如 '2026-06-24T13:00:00.000+08:00' 或 '2026-06-24T11:10:18.289Z'
 *    → 解析为绝对时间，转换为 Beijing 显示
 * 2. 无时区的 'YYYY-MM-DD HH:mm:ss' — 如 '2026-06-24 19:10:18'
 *    → 当作已经是 Beijing 时间，直接显示
 *    （来源：2026-06-24 用户希望 Notion 创建时间存 Beijing 格式）
 *
 * @param {string|null|undefined} input - 时间字符串
 * @returns {string} 格式化时间（如 "2026-06-24 13:00:00 (北京时间)"），无效返回 "N/A"
 */
function formatBeijingTime(input) {
  if (!input) return 'N/A'
  // 无时区格式：直接当作 Beijing 时间用
  // （避免依赖运行环境时区，必须在所有环境下都正确）
  if (typeof input === 'string' && /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(input)) {
    return `${input} (北京时间)`
  }
  // ISO 格式（带时区）：解析后转 Beijing
  const d = new Date(input)
  if (isNaN(d.getTime())) return 'N/A'
  const beijing = new Date(d.getTime() + 8 * 3600 * 1000)
  const formatted = beijing.toISOString().slice(0, 19).replace('T', ' ')
...
async function notifyDeadLetter(entry) {
  ...
  // 序列化在 try 外：如果 entry 含循环引用，JSON.stringify 会同步抛 TypeError。
  // 必须吞掉以保持"MUST NEVER reject"契约。失败时降级为只发 outTradeNo + error。
  // 订单时间（orderData.paidAt）格式化为北京时间，运营收到 webhook 时能直接看到下单时间
  // 来源：2026-06-24 运营需求（方便查日志/对账）
  const orderTime = formatBeijingTime(entry.orderData?.paidAt)
  let body
  try {
    body = JSON.stringify({
      message: `【订单失败】订单号: ${entry.outTradeNo}，订单时间: ${orderTime}，邮箱: ${entry.orderData?.email || 'N/A'}，姓名: ${entry.orderData?.name || 'N/A'}，商品: ${entry.orderData?.productName || 'N/A'}，金额: ${entry.orderData?.amount || 'N/A'}元，错误: ${entry.error}。请用 message 工具发 QQ 通知我，accountId 填 xingchen，target 填 qqbot:c2c:c6366d4f6511a4538de7abdf67c8483b`,
      name: 'notion-payment-dead-letter',
      accountId: 'xingchen',
      target: 'qqbot:c2c:c6366d4f6511a4538de7abdf67c8483b'
    })
  } catch (e) {
    console.error('[dead-letter] entry 序列化失败（疑似循环引用），降级发送', {
      error: e.message,
      outTradeNo: entry?.outTradeNo,
      errorMsg: entry?.error
    })
    body = JSON.stringify({
      message: `【订单失败】订单号: ${entry?.outTradeNo}，错误: ${entry?.error}。请用 message 工具发 QQ 通知我，accountId 填 xingchen，target 填 qqbot:c2c:c6366d4f6511a4538de7abdf67c8483b`,
      name: 'notion-payment-dead-letter',
      accountId: 'xingchen',
      target: 'qqbot:c2c:c6366d4f6511a4538de7abdf67c8483b'
    })
