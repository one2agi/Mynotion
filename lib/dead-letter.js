...
async function notifyDeadLetter(entry) {
  ...
  try {
    body = JSON.stringify({
      message: `【订单失败】订单号: ${entry.outTradeNo}，邮箱: ${entry.orderData?.email || 'N/A'}，姓名: ${entry.orderData?.name || 'N/A'}，商品: ${entry.orderData?.productName || 'N/A'}，金额: ${entry.orderData?.amount || 'N/A'}元，错误: ${entry.error}。请用 message 工具发 QQ 通知我，accountId 填 xingchen，target 填 qqbot:c2c:c6366d4f6511a4538de7abdf67c8483b`,
