 ...
 * - redis_fallback 用 30 天长 TTL,做"跨容器重启兜底层"
 *
 * 设计目的:
 * 当 Notion 拉取失败时,getOrSetDataWithCache 先查 Redis fallback(30 天内都还有),
 * 再查 stale(本地内存/文件,容器重启就丢)。这保证:
 * - 短时间 Notion 不可达:用 stale(快速)
 * - 容器重启 + Notion 仍不可达:用 Redis fallback(只要 30 天内成功过一次)
...
const FALLBACK_TTL_SEC = 30 * 24 * 60 * 60 // 30 天
...
export async function saveFallbackStrict(key, data) {
  if (!isRedisReady()) {
    throw new Error(`Redis fallback is unavailable for key:${key}`)
  }
  ...
  if (result !== 'OK') {
    throw new Error(`Redis fallback write was not acknowledged for key:${key}`)
  }
}
