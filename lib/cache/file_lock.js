...
const VALID_TIMEOUT_STRATEGIES = new Set(['bypass', 'wait', 'throw'])

function getLockPath(key) {
  return path.join(getBuildSessionPath('locks'), `${hashForCachePath(key)}.lock`)
}
...
function normalizeOptions(timeoutOrOptions) {
  ...
  const timeoutStrategy = VALID_TIMEOUT_STRATEGIES.has(
...
async function runWithOwnedLock(lockPath, payload, fn) {
...
async function waitForLockResult({
  key,
  lockPath,
  fn,
  readCache,
...
export async function withFileLock(
  ...
  if (timeoutStrategy === 'throw') {
    throw new Error(`[FileLock] timed out acquiring key:${key} after ${timeout}ms`)
  }
