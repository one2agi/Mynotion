...
export async function setDataToCacheStrict(key, data, customCacheTime) {
  if (!isUsableCacheValue(data)) {
    throw new Error(`Refusing to persist unusable cache data for key:${key}`)
  }

  const api = getApi()
  const write = api.setCacheStrict || (!hasRedis ? api.setCache : null)
  if (typeof write !== 'function') {
    throw new Error(`No strict cache writer is available for key:${key}`)
  }
  await write.call(api, key, data, customCacheTime)
}
