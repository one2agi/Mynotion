...
export async function getStaticPathsBase({
  ...
  return {
    paths: prerenderInIsrBuild
      ? allPages.filter(filterFn).map(mapPageToParams)
      : [],
    fallback: 'blocking'
  }
}

export function getLandingOnlyStaticPaths() {
  if (!isLandingSite()) return null
  return {
    paths: [],
