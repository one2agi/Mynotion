...
export async function fetchNotionPageBlocks(id, from = undefined, options = {}) {
  ...
  const loadFromSource = () =>
    limit(() => getPageWithRetry(id, from, 3, cacheKey, !options?.forceSource))
  const pageBlock = options?.forceSource
    ? await loadFromSource()
    : await getOrSetDataWithCache(cacheKey, loadFromSource)

  if (!pageBlock) {
    if (options?.forceSource) {
      throw new Error(
        `[fetchNotionPageBlocks] source returned no page data for id=${id}`
      )
    }
 ...
 * @returns
 */
export const fetchInBatches = async (ids, batchSize = 30, options = {}) => {
  ...
  if (options.strict) {
    const requestedIds = new Set(ids.map(normalizeBlockIdForComparison))
    const usableReturnedIds = new Set(
      Object.entries(fetchedBlocks)
        .filter(([key, entry]) => {
          const block = getBlockValue(entry)
          return (
            block?.id &&
            normalizeBlockIdForComparison(block.id) ===
              normalizeBlockIdForComparison(key)
          )
        })
        .map(([key]) => normalizeBlockIdForComparison(key))
    )
    const missingCount = [...requestedIds].filter(
      id => !usableReturnedIds.has(id)
    ).length
    if (missingCount) {
      throw new Error(
        `[fetchInBatches] incomplete response requested=${requestedIds.size} returned=${requestedIds.size - missingCount} missing=${missingCount}`
      )
    }
  }
...
function normalizeBlockIdForComparison(id) {
