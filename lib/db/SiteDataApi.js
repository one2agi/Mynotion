...
import { saveFallback } from '../cache/redis_fallback'
...
function fetchSiteDataWithRetry({ pageId, from, forceSource = false }) {
  return withRetry(
    () =>
      forceSource
        ? fetchSiteDataFromSource({ pageId, from, forceSource: true })
...
async function writeFreshCache(cacheKey, data) {
  ...
  await saveFallback(cacheKey, data)
}
...
export async function fetchFreshConfiguredGlobalData({
  ...
  for (let index = 0; index < fresh.length; index++) {
    ...
    await writeFreshCache(localeKey, item.data)

    // A locale-less normal read resolves the first configured database.
    if (index === 0 && item.locale) {
      const defaultKey = getGlobalDataCacheKey({
        pageId: configuredPageId,
        locale: undefined
      })
      if (defaultKey !== localeKey) await writeFreshCache(defaultKey, item.data)
    }
  }
