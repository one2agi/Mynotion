...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const isRouteReadable = async pageId => {
    try {
      return !(await isExplicitlyPrivate(pageId))
    } catch (e) {
  ...
  if (!post && typeof lastSegment === 'string' && /^[a-f0-9-]{32,36}$/i.test(lastSegment)) {
    ...
    if (await isRouteReadable(lastSegment)) {
  ...
  if (post?.id && !routeStateChecked) {
    if (!(await isRouteReadable(post.id))) post = null
  }
