...
import notionAPI from './notion/getNotionAPI'
...
export async function fetchGlobalAllData({
  ...
  data.latestPosts = cleanLatestPosts(data.latestPosts)
...
function cleanLatestPosts(posts) {
...
function cleanPostSummaries(posts) {
  if (!Array.isArray(posts)) return posts
  return posts.map(cleanPostSummary)
}

function cleanPostSummary(post) {
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  props.latestPosts = cleanLatestPosts(props.latestPosts)
...
async function convertNotionToSiteData(
  ...
  await hydrateSelectedCollectionViewQuery({
    pageRecordMap,
    collectionId,
    collectionView,
    viewIds,
...
function handleDataBeforeReturn(db) {
  ...
  db.latestPosts = cleanLatestPosts(cleanPages(db.latestPosts, db.tagOptions))
...
async function hydrateSelectedCollectionViewQuery({
  pageRecordMap,
  collectionId,
  collectionView,
  viewIds,
  ...
  if (!collectionId || !targetViewId || !targetView) return

  const limit = getCollectionReducerLimit()
  ...
  try {
    ...
    mergeRecordMap(pageRecordMap, collectionData?.recordMap)
    pageRecordMap.collection_query[collectionId] = {
      ...pageRecordMap.collection_query[collectionId],
      [targetViewId]: collectionData?.result?.reducerResults
    }

    const hasMore =
      collectionData?.result?.reducerResults?.collection_group_results?.hasMore
    if (hasMore) {
      console.warn(
        `[${traceId}] Notion collection query still has more rows after limit ${limit}. ` +
          'Increase NEXT_PUBLIC_NOTION_COLLECTION_REDUCER_LIMIT.'
      )
    }
  } catch (err) {
    console.warn(
      `[${traceId}] query selected Notion collection view failed:`,
      err?.message || err
    )
  }
}
...
function getCollectionViewValue(collectionView, viewId) {
  if (!collectionView || !viewId) return null

  const viewEntry = collectionView?.[viewId]
  return viewEntry?.value?.value || viewEntry?.value || null
}

function getCollectionReducerLimit() {
  const limit = Number(
    process.env.NEXT_PUBLIC_NOTION_COLLECTION_REDUCER_LIMIT ||
      process.env.NOTION_COLLECTION_REDUCER_LIMIT ||
      100000
  )

  return Number.isFinite(limit) && limit > 0 ? limit : 100000
}

function mergeRecordMap(targetRecordMap, sourceRecordMap) {
  if (!targetRecordMap || !sourceRecordMap) return

  targetRecordMap.block = {
    ...(targetRecordMap.block || {}),
    ...(sourceRecordMap.block || {})
  }
  targetRecordMap.collection = {
    ...(targetRecordMap.collection || {}),
    ...(sourceRecordMap.collection || {})
  }
  targetRecordMap.collection_view = {
    ...(targetRecordMap.collection_view || {}),
    ...(sourceRecordMap.collection_view || {})
  }
  targetRecordMap.notion_user = {
    ...(targetRecordMap.notion_user || {}),
