...
export async function resolvePostProps({
  ...
  const isRouteReadable = async (pageId, sourceConfirmed = false) => {
    try {
      ...
      if (allowSourceConfirmedWithoutRouteState && sourceConfirmed) {
        console.warn(
          `[${taskId}] [resolvePostProps] source-confirmed build fallback without route state`
        )
        return true
      }
  ...
  if (post?.id && !routeStateChecked) {
    if (!(await isRouteReadable(post.id, true))) post = null
  }
