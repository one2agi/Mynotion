...
import { fetchEventsFromOfficialAPI } from './notion/eventDataSource'
...
async function convertNotionToSiteData(
  ...
  const officialEvents = await fetchEventsFromOfficialAPI({
    typeProperty: BLOG.NOTION_PROPERTY_NAME.type,
    statusProperty: BLOG.NOTION_PROPERTY_NAME.status,
    typeValue: BLOG.NOTION_PROPERTY_NAME.type_event,
    statusValue: BLOG.NOTION_PROPERTY_NAME.status_publish
  })
  if (officialEvents.length > 0) {
    ...
    officialEvents.forEach(event => {
      if (!existingEvents.has(event.id) && !existingEvents.has(event.slug)) {
        collectionData.push(event)
      }
    })
  }
