...
const ClerkUserBridge = dynamic(() => import('@/components/ClerkUserBridge'), {
  ssr: false
})
...
export function GlobalContextProvider(props) {
  ...
  const [clerkUserState, setClerkUserState] = useState({
    isLoaded: true,
    isSignedIn: false,
    user: false
  })
  ...
  return (
    <GlobalContext.Provider
      value={contextValue}>
      {enableClerk && <ClerkUserBridge onChange={setClerkUserState} />}
      {children}
    </GlobalContext.Provider>
  )
}
