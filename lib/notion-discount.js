...
const DISCOUNT_RESERVATION_TTL_SECONDS = Number.parseInt(
  process.env.DISCOUNT_RESERVATION_TTL_SECONDS || '1800',
  10
)

function getDiscountReservationKey(pageId) {
  return `notionnext:discount-reservation:${pageId}`
}
...
async function reserveDiscountCode(pageId, outTradeNo, client) {
  const redis = client || redisClient
  if (!pageId || !outTradeNo) {
    throw new Error('reserveDiscountCode requires pageId and outTradeNo')
  }
  if (typeof redis?.set !== 'function') {
    throw new Error('Redis is required to reserve one-time discount codes')
  }

  const result = await withRetry(() =>
    redis.set(
      getDiscountReservationKey(pageId),
      outTradeNo,
      'EX',
      DISCOUNT_RESERVATION_TTL_SECONDS,
      'NX'
    )
  )
  return result === 'OK'
...
async function releaseDiscountReservation(pageId, client) {
  const redis = client || redisClient
  if (!pageId || typeof redis?.del !== 'function') return
  await withRetry(() => redis.del(getDiscountReservationKey(pageId)))
}
